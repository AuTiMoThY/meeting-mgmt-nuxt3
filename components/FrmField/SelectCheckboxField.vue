...
const handleOptionClick = (option) => {
    if (selectedValue.value.includes(option)) {
        ...
        selectedValue.value = [...selectedValue.value, option];
    }
};

// 獲取當前選中的值
const currentValue = computed(() => {
    return props.modelValue.length > 0 ? props.modelValue : "請選擇";
});
...
const isActive = (option) => {
    ...
    return (
        ...
        selectedValue.value.filter((item) => item.id === option.id).length > 0
    );
};
...
<template>
    <div class="frm_field frm_field-selectcheckbox">
            ...
            <div class="frm_field-ctrler">
                <div class="frm_dropdown">
                    <div class="frm_dropdown-inner" @click="toggleMenu">
                        <div class="txt">
                            <div class="current font-title-s txt-gray-6">
                    ...
                    <div ref="menu" class="frm_dropdown-menu">
                        <ul class="frm_dropdown-menu-list">
                            <li
                                ...
                                <template v-else>
                                    <RcxField
                                        :model-value="isActive(option)"
                                        :label="option.name"
                                        :rcx-id="`people-${index}`"
                                        @update:model-value="
                                            (checked) => handleCheckboxChange(option, checked)
                                        ">
                                    </RcxField>
                                </template>
                            </li>
